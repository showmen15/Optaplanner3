...
public class VehicleRoutingEasyScoreCalculator implements
EasyScoreCalculator<VehicleRoutingSolution> {

    public HardSoftScore calculateScore(VehicleRoutingSolution solution) {
        boolean timeWindowed = solution instanceof
TimeWindowedVehicleRoutingSolution;
        List<Customer> customerList = solution.getCustomerList();
        List<Vehicle> vehicleList = solution.getVehicleList();
        Map<Vehicle, Integer> vehicleDemandMap = new HashMap<Vehicle,
Integer>(vehicleList.size());
        Map<Vehicle, Integer> vehiclesCost = new HashMap<Vehicle,
Integer>(vehicleList.size());
        ...
        for (Customer customer : customerList) {
            ...
            if (previousStandstill != null) {
                ...
                vehicleDemandMap.put(vehicle,
vehicleDemandMap.get(vehicle) + customer.getDemand());
                // Score constraint distanceToPreviousStandstill
                softScore -= customer.getDistanceFromPreviousStandstill();
                if (customer.getNextCustomer() == null) {
                    // Score constraint distanceFromLastCustomerToDepot
                    softScore -=
customer.getLocation().getDistance(vehicle.getLocation());
...
//        int maxCost = 0;
//        for (Integer vehicleCost : vehiclesCost.values()) {
//            if (maxCost < vehicleCost)
//                    maxCost = vehicleCost;
//        }

//        for (Integer vehicleCost : vehiclesCost.values()) {
//            if (vehicleCost < maxCost)
//                maxCost += maxCost-vehicleCost;
//
//        }

//        System.out.println("    calculated fitness: "+maxCost);
        return HardSoftScore.valueOf(0, -maxCost);
    }
